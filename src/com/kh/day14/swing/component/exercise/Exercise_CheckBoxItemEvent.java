...
public class Exercise_CheckBoxItemEvent extends JFrame{
	...
	private String [] names = {"사과", "배", "체리"};  // 체크 박스 이름 배열
			...
			public void itemStateChanged(ItemEvent e) {       //아이템 선택될 때
				if(e.getStateChange() == ItemEvent.SELECTED) {    //선택이 됐으면
					if(e.getItem().equals(fruits[0])) {        //그 이름이 사과니?
						...
						sum += 500;                               //500을 더하렴
					} else if(e.getItem().equals(fruits[2])) {   //그이름이 체리니?
						sum += 2000;                                  //2000을 더하렴
					}	
					...
					if(e.getItem().equals(fruits[0])) {     //사과는 100빼고
						sum -= 100;
					} else if(e.getItem().equals(fruits[1])) {   //배는 500뺴고
						sum -= 500;
					} else if(e.getItem().equals(fruits[2])) {   //체리는 2000빼고
						sum -= 2000;
					}	
				}
...
//		c.add(cherry);
		...
		//세트1
		//ItemListener의 메소드가 한개밖에 없어서 어댑터도 없다.
