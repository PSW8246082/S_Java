...
public class Exercise_SliderChangeEvent extends JFrame{
	...
	public Exercise_SliderChangeEvent() {
		...
		colorLabel = new JLabel("SLIDER EXAMPLE"); //슬라이더 변화시겨 바뀐 색상을 나타냄
		
		
		sliders = new JSlider[3];  //슬라이더는 3개니까 객체배열로
		for(int i = 0; i < sliders.length; i++) {
			sliders[i] = new JSlider(JSlider.HORIZONTAL, 0, 255, 128);   //수평모양으로 최소 0, 최대255, 시작값 125 슬라이더 만듬
			...
			sliders[i].addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent e) {
					...
					// 라벨의 배경색을 RGB 값으로 바꿔서 컬러픽커처럼 보이게 함
