...
public class Exercise_ComboActionEvent extends JFrame{
	...
	public Exercise_ComboActionEvent() {
		...
		setTitle("ComboActionEvent 활용 예제");
		...
		c.add(strCombo);  //꼭 콘텐트팬에 추가해주세요.
		c.add(imgLabel);
		strCombo.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				JComboBox<String> strCb = (JComboBox<String>)e.getSource();  // e.getSource()는 이벤트가 발생한 콤포넌트를 가져옴(this와 비슷)
				int index = strCb.getSelectedIndex(); //선택된 아이템의 index 값을 구해오는 메소드인데 DOM에도 있음
