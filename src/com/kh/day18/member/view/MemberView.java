...
import com.kh.day18.member.controlloer.MemberController_prev;
		...
		while(true) {
			...
			switch(choice) {
				case 1 : 
					...
					break;
				case 2 : 
					name = this.inputMemberName();   //수정할 데이터의 이름 입력
					index = controller.findIndexByName(name);   //인덱스 값 찾음
					//수정하지 않은 부분은 기존 데이터가 유지되도록 정보를 불러옴
					mOne = controller.findOneByName(name);        //원래 저장된 데이터
					if(index != -1) {
						mOne = this.modifyMember(mOne);  //데이터가 있을 때 수정할 정보 입력
					}
					controller.modifyMember(index, mOne);          //데이터 수정
					break; 
				case 3 : 
					name = this.inputMemberName();  //삭제할 데이터의 이름 입력
					index = controller.findIndexByName(name);
					if(index != -1) {
						controller.subMember(index);           //삭제
					}          
					break;
				case 4 : 
